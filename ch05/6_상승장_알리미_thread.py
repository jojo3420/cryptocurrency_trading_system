    ...
    def run(self):
        while True:
            ...
            for ticker in tickers:
                ma3_data: set = self.get_market_info(ticker, 3)
                ma5_data: set = self.get_market_info(ticker, 5)
                # print(ma5_data)
                data[ticker] = ma3_data + ma5_data[1:]

            # print(data)
            self.finished.emit(data)  # emit(event) 발생
            self.msleep(500)  # 0.5초

    def get_market_info(self, ticker: str, days: int = 5) -> tuple:
        try:
            is_bull = '하락장'
